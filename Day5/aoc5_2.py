...
for input in inputs:
	...
	else:																		#for diagonal line
		#diagonal line which are -45deg
		if x1 < x2 and y1 < y2:
			for vent in range(x2 - x1 + 1):
				vents_map[y1 + vent][x1 + vent] += 1
		elif x1 > x2 and y1 > y2:
			for vent in range(x1 - x2 + 1):
				vents_map[y2 + vent][x2 + vent] += 1
